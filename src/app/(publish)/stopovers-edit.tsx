...
function Stopovers() {
  ...
  const { ride, polyline, createRide, loading, success, error,selectedPlaces ,setSelectedPlaces} = useCreateRideStore()
    ...
    <View className="flex-1 bg-white font-[Kanit-Regular]">
      ...
      <View className="absolute bottom-8 left-0 right-0 px-6 flex-row gap-4">
        <TouchableOpacity
          ...
          onPress={() => router.push({pathname:"/(publish)/add-city",params:{place:route?.params?.location,path:'/(publish)/stopovers-edit'}})}
          activeOpacity={0.8}
        >
