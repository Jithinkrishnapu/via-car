      ...
      <View className="flex-1 px-6 pt-16 pb-12">
        ...
        <View className="flex-row items-center justify-center gap-2 mt-10">
          ...
          <TextInput
            ...
            maxLength={2}
            className="bg-[#F5F5F5] text-[59px] font-[Kanit-Regular] text-[#3C3F4E] text-center w-[100px] h-[84px] rounded-lg"
          />
          ...
          <View className="ml-2 h-[84px] w-[55px] rounded-lg overflow-hidden border border-[#00665A]">
            {([t("time.am"), t("time.pm")] as const).map((p, i) => {
              const isSel = period === (p === t("time.am") ? "AM" : "PM");
              return (
                <TouchableOpacity
                  key={p}
                  onPress={() => setPeriod(p === t("time.am") ? "AM" : "PM")}
                  activeOpacity={0.8}
